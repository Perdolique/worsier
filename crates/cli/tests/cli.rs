...
#[test]
fn formats_global_comment_spacing_with_platform_line_endings() {
    let directory = tempfile::tempdir().unwrap();
    for newline in ["\n", "\r\n"] {
        let source =
            "first()\n// second\nsecond()\n\n\n// detached\n\n\nthird()".replace('\n', newline);
        ...
        write(
            &directory.path().join("worsier.jsonc"),
            r#"{"rules":{"commentSpacing":false}}"#,
        );
        let output = command(directory.path()).arg("sample.ts").output().unwrap();
        assert!(output.status.success(), "{}", stderr(&output));
        let compact = "first()\n// second\nsecond()\n// detached\n\nthird()".replace('\n', newline);
        assert_eq!(String::from_utf8(output.stdout).unwrap(), compact);
    }
}
...
fn supports_object_property_spacing_from_config() {
    ...
    write(
        &directory.path().join("worsier.jsonc"),
        r#"{"rules":{"commentSpacing":false,"importLayout":false}}"#,
    );
