...
export function parseHyperfineJson(text) {
  ...
  return Object.fromEntries(document.results.map((result) => {
    if (typeof result.command !== 'string' || !Array.isArray(result.times)) {
...
export function parsePeakRss(text, platform = process.platform) {
  if (platform === 'darwin') {
...
export function replaceGeneratedBlock(readme, generatedMarkdown) {
...
export async function describeFixture(path) {
...
export async function describeManifest(directory) {
...
export function assertSameManifest(expected, actual, label = 'Formatted output') {
  ...
  const summarize = (paths) => paths.length === 0 ? 'none' : paths.slice(0, 5).join(', ')
...
export function formatRelativeTime(seconds, baselineSeconds) {
...
export function buildRootBenchmarkBlock(report) {
  const tools = ['worsier', 'prettier', 'oxfmt']
...
export function buildDetailedReport(report) {
  const lines = [
  ...
  for (const scenarioName of ['small', 'parser', 'projectWrite', 'projectCheck']) {
    const scenario = report.scenarios[scenarioName]
    const baseline = fastestMedianSeconds(scenario)
    for (const tool of ['worsier', 'prettier', 'oxfmt']) {
...
function formatRootTiming(scenario, tool) {
...
function fastestMedianSeconds(scenario) {
...
async function visit(directory, found) {
