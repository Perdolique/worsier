...
const singleLineCallSpacing = await binding.format(
  'smoke.ts',
  "function complete() {\n  activePointerId = null\n  emit('complete')\n\n  holdTimer.stop()\n}",
  '{}'
)
assert.equal(
  singleLineCallSpacing,
  "function complete() {\n  activePointerId = null\n\n  emit('complete')\n  holdTimer.stop()\n}"
)
...
for (const newline of ['\n', '\r\n']) {
  ...
  const commentExpected = 'first()\n\n// second\nsecond()\n\n\n// detached\n\n\nthird()'.replaceAll('\n', newline)
  const commentOutput = await binding.format('smoke.ts', commentSource, '{}')
  assert.equal(commentOutput, commentExpected)
  const commentRepeated = await binding.format('smoke.ts', commentOutput, '{}')
  assert.equal(commentRepeated, commentOutput)
  const commentDisabled = await binding.format('smoke.ts', commentSource, '{"rules":{"commentSpacing":false}}')
  const compactExpected = 'first()\n// second\nsecond()\n// detached\n\nthird()'.replaceAll('\n', newline)
  assert.equal(commentDisabled, compactExpected)
}
